...
for d in d_vec:
    data_file = os.path.join(script_dir, f"data_pymatching_d{d}.txt")
    with open(data_file, "w", encoding="utf8") as data_f:
        data_f.write("<T> <average_decoding_time> <average_decoding_time_per_round> <average_decoding_time_per_defect>\n")
        ...
        for T in T_vec:
            ...
            with open(syndrome_file_path, "r", encoding='utf8') as f:
                ...
                assert initializer.vertex_num == (T + 1) * d * (d+1)
            ...
            # run simulation
            raw_time_file = os.path.join(tmp_dir, f"raw_time_d{d}_T{T}.txt")
            ...
            average_decoding_time_per_round = average_decoding_time / (T + 1)
            if PYMATCHING_BATCH_DECODING:
