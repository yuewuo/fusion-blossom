...
tmp_dir = os.path.join(script_dir, "raw-data-2023-04-09")
...
with open(data_file, "w", encoding="utf8") as f:
    f.write("<measure_interval> <average_latency> <stddev_latency> <sample_latency>\n")
    for idx, measure_interval in enumerate(measure_interval_vec):
        ...
        print(f"measure_interval {measure_interval}: average {average_latency}, stddev {stddev_latency}")
        f.write("%.5e %.5e %.3e %s\n" % (
            measure_interval,
            average_latency,
            stddev_latency,
