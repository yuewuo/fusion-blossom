...
impl<SerialModule: DualModuleImpl + Send + Sync> DualModuleParallel<SerialModule> {
    ...
    pub fn new_config(initializer: &SolverInitializer, mut partition_config: PartitionConfig, config: DualModuleParallelConfig) -> Self {
        ...
        if partition_config.partitions.len() == 0 {
            partition_config.partitions = vec![VertexRange::new(0, initializer.vertex_num)];
        }
        let mut units = vec![];
        let partition_info = Arc::new(PartitionInfo::new(&partition_config, initializer));
        let unit_count = partition_config.partitions.len() + partition_config.fusions.len();
        ...
        let partition_units: Vec<PartitionUnitPtr> = (0..unit_count).map(|unit_index| PartitionUnitPtr::new(PartitionUnit {
            ...
            enabled: unit_index < partition_config.partitions.len(),
        })).collect();
        let mut partitioned_initializers: Vec<PartitionedSolverInitializer> = (0..unit_count).map(|unit_index| {
            ...
            PartitionedSolverInitializer {
                ...
                owning_interface: if unit_index < partition_config.partitions.len() { None } else { Some(partition_units[unit_index].downgrade()) },
        ...
        for (edge_index, &(i, j, weight)) in initializer.weighted_edges.iter().enumerate() {
            ...
            if config.edges_in_fusion_unit {
                ...
                if ancestor_unit_index < partition_config.partitions.len() {
                    ...
                    dfs_add(descendant_unit_index, &partition_config, &partition_info, i, j, weight, &contained_vertices_vec, &mut partitioned_initializers, edge_index);
                }
            }
        }
...
impl<SerialModule: DualModuleImpl + Send + Sync> DualModuleImpl for DualModuleParallel<SerialModule> {
    ...
    fn new(initializer: &SolverInitializer) -> Self {
        Self::new_config(initializer, PartitionConfig::default(), DualModuleParallelConfig::default())
    }
    ...
    fn clear(&mut self) {
        self.thread_pool.scope(|_| {
            self.units.par_iter().enumerate().for_each(|(unit_idx, unit_ptr)| {
                let mut unit = unit_ptr.write();
                unit.clear();
                unit.is_active = unit_idx < self.partition_config.partitions.len();  // only partitioned serial modules are active at the beginning
...
pub mod tests {
    ...
    pub fn dual_module_parallel_basic_standard_syndrome_optional_viz<F>(mut code: impl ExampleCode, visualize_filename: Option<String>, mut syndrome_vertices: Vec<VertexIndex>
        ...
        let mut dual_module = DualModuleParallel::new_config(&initializer, partition_config, config);
