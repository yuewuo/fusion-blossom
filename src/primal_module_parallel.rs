...
pub struct PrimalModuleParallelUnit {
    ...
    pub interface: DualModuleInterface,
    /// partition information generated by the config
    pub partition_info: Arc<PartitionInfo>,
...
impl PrimalModuleParallel {
    ...
    pub fn parallel_solve_visualizer<DualSerialModule: DualModuleImpl + Send + Sync + FusionVisualizer>
        ...
        if let Some(visualizer) = visualizer {
            ...
            visualizer.snapshot_combined(format!("solved"), vec![&last_unit.interface, parallel_dual_module, self]).unwrap();
        } else {
    ...
    fn iterative_solve_step_callback<DualSerialModule: DualModuleImpl + Send + Sync, F: Send + Sync>(&self, primal_module_parallel: &PrimalModuleParallel
        ...
        if let Some((left_child_weak, right_child_weak)) = primal_unit.children.as_ref() {
            ...
            if let Some(callback) = callback.as_mut() {  // do callback before actually breaking the matched pairs, for ease of visualization
                callback(&primal_unit.interface, &dual_unit, &primal_unit.serial_module, None);
            }
            primal_unit.break_matching_with_mirror(dual_unit.deref_mut());
            let (serial_module, interface) = primal_unit.split_borrow_serial_module_interface();
            for syndrome_index in syndrome_range.iter() {
                ...
                serial_module.load_syndrome(syndrome_vertex, interface, dual_unit.deref_mut());
            }
            serial_module.solve_step_callback_interface_loaded(interface, dual_unit.deref_mut()
                , |interface, dual_module, primal_module, group_max_update_length| {
            ...
            let (serial_module, interface) = primal_unit.split_borrow_serial_module_interface();
            serial_module.solve_step_callback(interface, &syndrome_pattern, dual_unit.deref_mut()
                , |interface, dual_module, primal_module, group_max_update_length| {
                    if let Some(callback) = callback.as_mut() {
                        callback(interface, dual_module, primal_module, Some(&group_max_update_length));
                    }
...
impl PrimalModuleParallelUnit {
                ...
                if let Some((match_target, _)) = &primal_node.temporary_match {
                    if let MatchTarget::VirtualVertex(vertex_index) = match_target {
                        if self.partition_info.vertex_to_owning_unit[*vertex_index] == self.unit_index {
                            ...
                            self.interface.set_grow_state(&primal_node.origin.upgrade_force(), DualNodeGrowState::Grow, dual_module);
                        }
                    }
                }
            }
    ...
    /// mutably borrow the serial module and the interface at the same time
    pub fn split_borrow_serial_module_interface(&mut self) -> (&mut PrimalModuleSerial, &mut DualModuleInterface) {
        (&mut self.serial_module, &mut self.interface)
    }
